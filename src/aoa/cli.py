...
def cmd_run(cfg: Config) -> int:
    ...
    if result.halted:
        return 1
    return _cycle_exit_code(result.cycle) if result.cycle else 0
...
def cmd_loop(cfg: Config) -> int:
    ...
    _print_environment(cfg)
    print(
        f"Starting continuous loop (team mode): cadence={cfg.cycle_seconds}s. Ctrl-C to stop."
    )
    try:
        while True:
            if team.broker.is_market_open():
                ...
                _print_team(result)
                if result.halted:
                    print("Cycle halted — see CEO review above.", file=sys.stderr)
