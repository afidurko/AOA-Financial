...
def test_orchestrator_adapts_and_learns(fake_broker, fake_llm, tmp_path):
    ...
    # Cycle 1: no prior to learn from, but signals are recalibrated (tagged).
    r1 = orch.run_cycle()
    sigs1 = r1.blackboard.signals_for("AAPL")
    assert sigs1 and all("adapted" in s.tags for s in sigs1)
...
def test_orchestrator_without_adapter_unchanged(fake_broker, fake_llm, tmp_path):
    ...
    r = orch.run_cycle()
    sigs = r.blackboard.signals_for("AAPL")
    assert sigs and all("adapted" not in s.tags for s in sigs)
