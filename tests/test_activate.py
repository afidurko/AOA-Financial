...
def test_ensure_profile_defaults_to_moomoo(monkeypatch, tmp_path):
    profiles = tmp_path / "profiles"
    profiles.mkdir()
    (profiles / "moomoo.env").write_text("AOA_ENV=paper-dry\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("AOA_PROFILE", raising=False)
    monkeypatch.delenv("AOA_ENV", raising=False)
    ensure_profile()
