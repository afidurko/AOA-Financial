#!/usr/bin/env bash
# Connect an external repo/workspace to AOA's shared second brain.
# The workspace mesh (brain/mesh/repos.yaml + write-aoa-workspace.sh) covers
# sibling clones beside this repo; this connects a project living ELSEWHERE on
# disk: it links the shared obsidian-second-brain (+ spine, obsidian-skills)
# Cursor skills, points that workspace's MCP + .env at the shared AOA-Vault, so
...
echo ""
echo "Open everything in one Cursor window (add this repo as an extra root):"
echo "  ./scripts/write-aoa-workspace.sh AOA.code-workspace \"$TARGET\""
